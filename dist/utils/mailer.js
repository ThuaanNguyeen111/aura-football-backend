...
function generateOTPEmailHTML(name, otpCode, message) {
    return `
  <!DOCTYPE html>
  <html lang="vi">
    <head>
...
async function sendOtpVerificationEmail(toEmail, name, otpCode) {
    ...
    await transporter.sendMail({
        ...
        html: generateOTPEmailHTML(name, otpCode, 'Cảm ơn bạn đã lựa chọn hệ thống đặt sân bóng Aura Football. Vui lòng nhập mã số OTP dưới đây vào ứng dụng di động để kích hoạt tài khoản của bạn:')
    });
}
async function sendForgotPasswordEmail(toEmail, name, otpCode) {
    const transporter = nodemailer_1.default.createTransport({
        service: 'gmail',
        auth: {
            user: process.env.SMTP_USER,
    ...
    await transporter.sendMail({
        ...
        html: generateOTPEmailHTML(name, otpCode, 'Chúng tôi đã nhận được yêu cầu khôi phục mật khẩu từ bạn. Hãy sử dụng mã OTP dưới đây để tiến hành thiết lập lại mật khẩu mới trên ứng dụng:')
    });
}
